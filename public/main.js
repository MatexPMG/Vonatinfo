...
function init() {
    ...
    const ormStandard = L.tileLayer('https://vinfo-production.up.railway.app/tiles/{z}/{x}/{y}.png', { attribution: attr });
...
srchIn.addEventListener('keydown', e => {
  ...
  if (e.key === 'Enter') {
    ...
    if (activeIndex >= 0 && items[activeIndex]) {
      selectedName = items[activeIndex].dataset.value;
    }
    else {
      const input = srchIn.value.trim();
      if (tMarkers.has(input)) {
        selectedName = input;
      } else {
        selectedName = Array.from(tMarkers.keys())
...
function showTrainInfo(train) {
  ...
  const og = train.trip?.departureStoptime?.stop.name || '';
  ...
  const rows = train.trip.stoptimes.map((stop, i, arr) => {
    ...
    return `
      <tr class="${rowClass}" id="station-row-${i}">
        <td id="stationN" rowspan="2"><a href="https://jegy.mav.hu/menetrend/allomas?a=${(stop.stop.gtfsId).split(":")[1].split("_")[0]}" target="_blank">${stop.stop.name}</a></td>
